...
class MisionDron(Node):
    ...
    def _save_photo(self, idx):
        ...
        if self.last_saved_stamp is not None and stamp_key == self.last_saved_stamp:
            self.get_logger().warn(
                f'[WP{idx}] Frame de cámara aún no actualizado, pero guardaré la foto de todos modos para no trabarme.'
            )
            # Removemos el "return False" para que NUNCA se trabe esperando.
        ...
        try:
            ...
            elif enc in ('mono8', 'mono'):
                img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
            # Si es bgr8, ya es BGR.
            
            # Ahora img es garantizadamente BGR correcto.
            if self.invert_colors:
                # Intercambiar canales Rojo y Azul (BGR a RGB) para emular el defecto del Tello.
                # Como OpenCV asume que le damos BGR, al pasarle RGB guardara R y B invertidos.
                img = img[:, :, [2, 1, 0]].copy()
        except Exception as exc:
        ...
        # Guardar JPG con defecto moderado (calidad 40 en lugar de 10)
        path = self.fotos_dir / f'wp_{idx:02d}.jpg'
    ...
    def _postprocess(self):
        ...
        # Usamos seam_cut para eliminar el efecto fantasma (paralaje) en objetos altos.
        cmd = ['python3', '-m', 'mision.stitch_pose', '--input', str(self.fotos_dir), '--output', str(stitch_out), '--camera', str(camera_yaml), '--blend-mode', 'seam_cut']
