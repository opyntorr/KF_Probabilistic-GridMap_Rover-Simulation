...
def main():
    ...
    ap.add_argument("--camera", type=Path, help="Path to camera YAML for intrinsic matrix")
    ...
    # Coverage-normalized vote fraction: corners/edges    # Decidir si invertir colores (Isaac Sim usa líneas grises sobre fondo blanco para que Canny agarre,
    # pero queremos procesar como si fueran cintas de color)
    is_sim = "sim" in str(getattr(args, "camera", "")).lower() or "sim" in str(args.input).lower()
