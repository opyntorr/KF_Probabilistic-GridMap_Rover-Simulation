...
def main():
    ...
    photos = sorted(list(in_dir.glob('*.png')) + list(in_dir.glob('*.jpg')))
    if not photos:
    ...
    for ph in photos:
        ...
        photos_data.append({'ph': ph, 'meta': meta, 'yaw': yaw})

    if args.use_image_yaw:
        from .preprocess import clahe_lab, estimate_tile_yaw
        for pd in photos_data:
            img = cv2.imread(str(pd['ph']))
            if img is not None:
                bgr = clahe_lab(img)
                det = estimate_tile_yaw(bgr)
                if det is not None:
                    pd['yaw'] = math.radians(det)
        
        # Smooth yaws
        from scipy.ndimage import median_filter
        yaws_deg = np.array([math.degrees(pd['yaw']) for pd in photos_data])
        smoothed = median_filter(yaws_deg, size=3, mode='nearest')
        for pd, s_yaw, orig in zip(photos_data, smoothed, yaws_deg):
            if abs(s_yaw - orig) > 5.0:
                pd['yaw'] = math.radians(s_yaw)
    ...
    obs_mask = cv2.inRange(hsv, lower_blue, upper_blue)
    
    explored = (weight_sum > 0.05).astype(np.uint8) * 255
    occ = np.full(canvas.shape[:2], 205, dtype=np.uint8)
    occ[explored == 255] = 254
    occ[obs_mask == 255] = 0
    
    # Cierre para rellenar huecos en cajas
    kernel = np.ones((7,7), np.uint8)
    obs_closed = cv2.morphologyEx((occ == 0).astype(np.uint8), cv2.MORPH_CLOSE, kernel)
    occ[obs_closed == 1] = 0
    
    # Invertir verticalmente para mantener consistencia con ROS y stitch_pose
    occ_flipped = cv2.flip(occ, 0)
    cv2.imwrite(str(out_dir / 'occupancy_map.pgm'), occ_flipped)
    cv2.imwrite(str(out_dir / 'mosaic_pose.png'), canvas)
    ...
    with open(out_dir / 'occupancy_map.yaml', 'w') as f:
        yaml.dump(ros_yaml, f)

    print(f'[stitcher] OK -> {panorama_path} y occupancy_map.pgm ({W}x{H} px, {args.resolution} m/px, '
